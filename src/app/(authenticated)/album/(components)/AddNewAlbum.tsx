...
export function AddNewAlbum({ type }) {
  ...
  const onSubmit = async () => {
    ...
    if (type === "comic") {
      ...
      await createNewComicAlbum(data).then((res) => {
        toast.success("Thêm album mới thành công");
        setIsLoading(false);
      });
    } else {
      const data = {
      ...
      await createNewAnimeAlbum(data).then((res) => {
        toast.success("Thêm album mới thành công");
        setIsLoading(false);
      });
    }
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody>
                <p>Bạn có chắc chắn muốn thêm thông tin album này</p>
              </ModalBody>
              <ModalFooter>
                <Button
                  color="success"
