    ...
    export function SearchAndActionBar({ setSearchWord, setSort, setIsLoaded }) {
        ...
        const sortedData = useMemo(() => {
            ...
            return [...data].sort((a, b) => {
                const aTotalPoints = a.challenges.reduce((acc, curr) => acc + curr.point, 0);
                const bTotalPoints = b.challenges.reduce((acc, curr) => acc + curr.point, 0);
            ...
            (user, columnKey) => {
                ...
                switch (columnKey) {
                    ...
                    case "point":
                        return (
                            <TableCell>
                                {user.challenges[0].point}
                            </TableCell>
                        );
                    case "time":
                        return (
                            <TableCell>
                                {user.challenges[0].time}
                            </TableCell>
                        );
                    default:
            ...
            <>
                <Modal size={'3xl'} isOpen={isOpen} onClose={onClose}>
                    <ModalContent>
                        {(onClose) => (
                            <>
                                <ModalHeader className="flex flex-col gap-1">Bảng xếp hạng</ModalHeader>
                                <ModalBody>
                                    <Table className="rounded-sm mt-4" aria-label="Leaderboard table">
