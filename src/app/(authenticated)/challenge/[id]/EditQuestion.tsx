...
export function EditQuestion({ questionId }) {
    ...
    const { deleteQuestion, editQuestion, fetchQuestionById } = useChallenge();
    ...
    useEffect(() => {
        const fetchQuestion = async () => {
            ...
            await fetchQuestionById(questionId).then((res) => {
                setQuestionName(res[0].questionName);
                setDefaultQuestionImage(res[0].mediaUrl);
                setAnswers(res[0].answers);
                setCorrectAnswerID(res[0].correctAnswerID);
            });
            setIsLoading(false);
        }
        fetchQuestion();
